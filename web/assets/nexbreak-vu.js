/**
 * nexbreak-vu.js — shared Web-Audio VU + program/playout for Player / Multiview.
 *
 ...
 *   nexbreak-vu-on          1 | 0               (show/hide meter overlay)
 *   nexbreak-vu-scale       1 | 0               (show dBFS scale beside meters)
 *   nexbreak-audio-muted    1 | 0               (Web Audio listen off)
 *   nexbreak-audio-volume   0..1               (Web Audio master gain)
 *   nexbreak-audio-program  main | sap
 *   nexbreak-audio-playout  stereo | surround   (5.1 → stereo mixdown vs discrete)
 *   nexbreak-vu-solo        -1 | channel index  (engineering solo)
 */
(function (global) {
  "use strict";

  const PREF_VISIBLE = "nexbreak-vu-on";
  const PREF_SCALE = "nexbreak-vu-scale";
  const PREF_MUTED = "nexbreak-audio-muted";
  const PREF_VOLUME = "nexbreak-audio-volume";
  const PREF_SOLO = "nexbreak-vu-solo";
  const PREF_PROGRAM = "nexbreak-audio-program";
  const PREF_PLAYOUT = "nexbreak-audio-playout";
  const MAX_CH = 8;
  ...
  function ensureStyles() {
    if (document.getElementById("nexbreak-vu-css")) return;
    const s = document.createElement("style");
    s.id = "nexbreak-vu-css";
    s.textContent = `
.nexbreak-vu {
...
.nexbreak-vu[hidden] { display: none !important; }
.nexbreak-vu-toolbar {
  display: flex; gap: 3px; justify-content: flex-end; flex-wrap: wrap;
}
.nexbreak-vu-toolbar button {
...
.nexbreak-vu-toolbar button:hover { color: var(--text, #d6dde6); border-color: var(--acc, #56c4f5); }
.nexbreak-vu-toolbar button.active {
  color: var(--on-acc, #08131a); background: var(--acc, #56c4f5);
  border-color: var(--acc, #56c4f5); font-weight: 600;
}
.nexbreak-vu-toolbar button:disabled { opacity: .35; cursor: not-allowed; }
.nexbreak-vu-meter-row {
...
.nexbreak-vu-scale[hidden] { display: none !important; }
.nexbreak-vu-scale-mark {
  position: absolute; left: 0; right: 2px; text-align: right;
  transform: translateY(50%);
  white-space: nowrap;
}
.nexbreak-vu-scale-unit {
...
.nexbreak-vu.show-scale .nexbreak-vu-track {
...
.nexbreak-vu-label { font-size: 9px; color: var(--dim, #98a6b5); }
.nexbreak-vu-ch.solo .nexbreak-vu-label { color: var(--acc, #56c4f5); font-weight: 600; }
`;
  ...
  function attach(opts) {
    ...
    // stereoOnly (NexBreak Roll): L/R meters + dB/ALL only — no Main/SAP/5.1.
    const stereoOnly = !!(opts && opts.stereoOnly);
    const root = document.createElement("div");
    root.className = "nexbreak-vu" + (stereoOnly ? " stereo-only" : "");
    root.hidden = true;
    root.innerHTML =
      '<div class="nexbreak-vu-toolbar">' +
      (stereoOnly
        ? ""
      ...
      "</div>" +
      '<div class="nexbreak-vu-meter-row">' +
      '<div class="nexbreak-vu-scale" hidden aria-hidden="true"></div>' +
      '<div class="nexbreak-vu-bars" role="group" aria-label="Audio level meters"></div>' +
      "</div>";
    container.appendChild(root);

    const barsEl = root.querySelector(".nexbreak-vu-bars");
    const scaleEl = root.querySelector(".nexbreak-vu-scale");
    ...
    let layout = layoutInfo(stereoOnly ? "stereo" : (opts.layout || "stereo"));
    ...
    function paintScale() {
      ...
      let html = '<span class="nexbreak-vu-scale-unit">dBFS</span>';
      for (const db of SCALE_MARKS_DB) {
        const bottom = heightFromDb(db);
        const label = db === 0 ? "0" : String(db);
        html += '<span class="nexbreak-vu-scale-mark" style="bottom:' +
    ...
    function paintToolbar() {
      ...
      if (btnMain) btnMain.classList.toggle("active", prog === "main");
      if (btnSap) {
      ...
      if (btnStereo) {
        ...
        if (!layout.has51) btnStereo.classList.remove("active");
      }
      if (btnSurround) {
        btnSurround.classList.toggle("active", play === "surround");
        btnSurround.disabled = !layout.has51;
        if (!layout.has51) btnSurround.classList.remove("active");
      }
      btnScale.classList.toggle("active", scaleOn);
    ...
    function rebuildMeterDom() {
      ...
      for (let i = 0; i < channelCount; i++) {
        ...
        btn.type = "button";
        btn.className = "nexbreak-vu-ch";
        btn.title = "Solo " + lab + " (this browser only)";
        btn.setAttribute("aria-label", "Solo audio " + lab);
        btn.setAttribute("aria-pressed", "false");
        btn.innerHTML =
          '<span class="nexbreak-vu-track">' +
          '<span class="nexbreak-vu-fill"></span>' +
          '<span class="nexbreak-vu-peak" style="bottom:0%"></span>' +
          "</span>" +
          '<span class="nexbreak-vu-label">' + lab + "</span>";
        ...
        chBtns.push(btn);
        fills.push(btn.querySelector(".nexbreak-vu-fill"));
        peaks.push(btn.querySelector(".nexbreak-vu-peak"));
      }
    ...
    function buildGraph(stream) {
      ...
      try {
        ...
        console.warn("nexbreak-vu: graph failed", err);
        teardownGraph();
        hasAudio = false;
        updateRootVisibility();
      }
        ...
        .then(() =>
          pc.setRemoteDescription({
            ...
            sdp:
              ...
              "c=IN IP4 0.0.0.0\r\n" +
              "a=ice-ufrag:nexbreak\r\n" +
              "a=ice-pwd:nexbreakmultiopusprobe000000\r\n" +
              "a=fingerprint:sha-256 " +
              "00:00:00:00:00:00:00:00:00:00:00:00:00:00:00:00:" +
              "00:00:00:00:00:00:00:00:00:00:00:00:00:00:00:00\r\n" +
              "a=setup:active\r\n" +
